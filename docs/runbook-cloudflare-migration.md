...
- [x] **予約 CRUD — 検証済み ✅（workerd/wrangler dev＋Neon ブランチ `verify-crud`（本番コピー・使い捨て）、2026-07-05）**。作成（POST /api/reserves 201→DB行実確認→マイ予約/カレンダーに描画）→ **重複チェック（同一期間 POST→409「この期間にはすでに予約が入っています。」）** → キャンセル（UI 2度押し確認→DB行消滅）まで一気通貫。※検証時に予約行の user_id が実在しない値（旧テストユーザー）で付いたが、これは **body 信頼のバグではない**——POST /api/reserves は `112fd00`（fix ブランチ群 2026-07-05）で **user_id のセッション導出＋401 ガードに修正済み**で、その修正は本番にもデプロイされている。真因は検証環境の `AUTH_URL` 欠如により API ルートが残存ゾンビ cookie 側のセッションを読んだこと（付録D「第3症状」と同根）。コードは正しくセッション由来の user_id を使っていた。「なりすまし残課題」は解消済み。
- [x] **機材／タグ CRUD — 検証済み ✅（同上、2026-07-05）**。**機材（lists）**: 登録（POST 201）→編集（PUT 200・DB反映確認）→削除（DELETE 200・DB消滅）を管理者 UI から実施（UI は `x-manager-key` ヘッダを付ける＝**key 経路**）。**タグ（tags）**: 作成（201）→更新（200）→削除（200）を **role=ADMIN 経路**（ヘッダ無し）で確認。**アップロード（/api/upload）**も role 経路で 200・`{url}` 形状・`safeName` サニタイズ（日本語/空白/括弧→`_`）・UUID 一意化を確認。⚠️ **role 経路は `AUTH_URL` 必須**（下の付録D 追記参照。無いと全て 403）。
- [x] 画像アップロード（新規、R2 に入る＋一意キー）— **本番 e2e 完了 ✅（2026-07-05 カットオーバー時）**: 管理者ページから画像アップロード→機材登録→表示まで本番ドメインで目視確認（workerd/preview 検証は 2-1 参照）
...
- **真因A の第3の症状: `AUTH_URL` 無しだと API ルートの role 判定が全滅する（重大・2026-07-05 preview で実証）**。既知の症状（①session=null ②ログアウト不能）に加え、**middleware を経由する API ルート（`/api/lists`・`/api/tags`・`/api/upload` 等）の中で `auth()` がセッション cookie を復号できず null になる**。結果、`hasManagerAccess` の role=ADMIN 経路が常に false → 管理操作が全て 403。切り分けの決め手は2つ: (a) middleware **除外**の `/api/auth/session` は正常に role=ADMIN を返すのに、middleware **経由**の API だけ落ちる。(b) 403 の応答時間が session 生成（DB 2クエリ≈700ms+）より明らかに速い（≈400ms）＝復号を試みて即座に失敗している。`.dev.vars` に `AUTH_URL=http://localhost:8787` を追加して再起動しただけで同一リクエストが 201 になることを確認。**UI 経由の機材 CRUD が動いていても安心できない**（UI は `x-manager-key` ヘッダを付けるため key 経路で通ってしまい、role 経路の破綻が隠れる）。本番カットオーバー時の `AUTH_URL` 投入は logout のためだけでなく **管理 API の role 認可のためにも必須**。
