...
generator client {
  ...
  runtime  = "cloudflare"             // ← 追加。adapter 経路にし、実行時 WASM コンパイルを回避
  output   = "../lib/generated/prisma" // ← 明示 output が必須になる
}

datasource db {
  provider  = "postgresql"
  url       = env("DATABASE_URL")     // プール接続（ホスト名に -pooler を含む）
  directUrl = env("DIRECT_URL")       // 直結（migrate / db pull 用）
}
```

`lib/db.ts` を **リクエストごと生成**へ書き換え（Workers は接続の使い回しが禁止）。現状のグローバル・シングルトンは廃止:

```ts
// before: export const db = globalThis.prisma || new PrismaClient();
...
> **churn 注意**: 現状は各所で `import { db }` を使っている。`getDb()` 化に伴い**全呼び出し箇所の修正**が要る。さらに generator 変更で `@prisma/client` からの型 import（`PrismaClient` / `UserRole` 等。`lib/api-auth.ts`, `auth.ts`, `data/*.ts` ほか）を**すべて生成先パスへ差し替える**。`grep -rn "@prisma/client"` で洗い出す。`@auth/prisma-adapter` の配線も同様。
>
> **テストも壊れる**: この import 差し替えはアプリコードだけでなく **vitest のテスト**（`actions/login.test.ts`, `data/*.test.ts`, `lib/*.test.ts` 等で `@prisma/client` や `db` を参照するもの）と `vitest.config.ts` のパス解決にも及ぶ。`grep -rn "@prisma/client"` の対象にテストも含めること。
>
> **安心材料（DB スキーマ変更なし）**: generator の変更・`directUrl` 追加は**モデル定義を一切変えない**ため、`prisma migrate` は走らない（Neon 本番 DB に対する破壊的マイグレーションは発生しない）。あくまで生成コードと接続設定の変更。
...
| 対象 | 変更内容 |
|---|---|
| `package.json` | +`@opennextjs/cloudflare` +`wrangler` +`@prisma/adapter-neon`、`prisma`/`@prisma/client`→`^6.19`、`bcryptjs`→`^3`、−`@vercel/blob` −`@vercel/speed-insights`、scripts に preview/deploy |
| `wrangler.jsonc`（新規） | main / compatibility_date(2025-04-01+) / flags[nodejs_compat, global_fetch_strictly_public] / assets / r2_buckets（+任意 images） |
| `open-next.config.ts`（新規） | `defineCloudflareConfig({})`（ISR 使用時のみ cache 設定追加） |
| `prisma/schema.prisma` | generator を `prisma-client`＋`runtime="cloudflare"`＋`output`、datasource に `directUrl` |
| `lib/db.ts` | グローバル singleton → `getDb()`（リクエストごとに adapter＋client 生成） |
| `@prisma/client` を import する全ファイル | 生成先パス（`@/lib/generated/prisma`）へ差し替え（`lib/api-auth.ts`, `auth.ts`, `data/*.ts` ほか。`grep -rn "@prisma/client"`） |
...
- **コスト**: Free に収まるかは Worker 圧縮サイズ次第。超えると月 $5。現状 Vercel Hobby ならむしろ増額になり得る（ADR「理由」参照）。
