...
- [x] **予約 CRUD — 検証済み ✅（workerd/wrangler dev＋Neon ブランチ `verify-crud`（本番コピー・使い捨て）、2026-07-05）**。作成（POST /api/reserves 201→DB行実確認→マイ予約/カレンダーに描画）→ **重複チェック（同一期間 POST→409「この期間にはすでに予約が入っています。」）** → キャンセル（UI 2度押し確認→DB行消滅）まで一気通貫。※既知の残課題（reserves POST が client 送信の `user_id` を信頼する）が実地でも確認された：サーバはセッションでなく body の user_id で行を作る。移行の回帰ではない（Vercel 時代からの挙動）が、カットオーバー後の課題として残る。
